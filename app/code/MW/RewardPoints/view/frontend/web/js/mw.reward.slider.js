define([
    ...
    window.MwRewardSlider = Class.create();
    ...
    Control.Slider = Class.create({
      initialize: function(handle, track, options) {
        ...
        if (this.allowedValues) {
          this.minimum = Math.min.apply(null, this.allowedValues);
          this.maximum = Math.max.apply(null, this.allowedValues);
        }
      ...
      getNearestValue: function(value){
        if (this.allowedValues){
          if (value >= Math.max.apply(null, this.allowedValues)) return(Math.max.apply(null, this.allowedValues));
          if (value <= Math.min.apply(null, this.allowedValues)) return(Math.min.apply(null, this.allowedValues));
      ...
      drawSpans: function() {
        ...
        if (this.options.startSpan)
          this.setSpan(this.options.startSpan,
            $R(0, this.values.length>1 ? Math.min.apply(null, this.getRange(0)) : this.value ));
        if (this.options.endSpan)
          this.setSpan(this.options.endSpan,
            $R(this.values.length>1 ? Math.max.apply(null, this.getRange(this.spans.length-1)) : this.value, this.maximum));
      },
