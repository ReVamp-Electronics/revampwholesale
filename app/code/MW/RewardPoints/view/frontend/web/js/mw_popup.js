define([
  ...
  MwPopup.prototype = {
    initialize: function(popup, link) {
      ...
      options.duration = this.first_value(options.duration, MwPopup.duration, 0.5);
      ...
      options.opacity = this.first_value(options.opacity, MwPopup.opacity, 0.5);
      options.show_delay = this.first_value(options.show_delay, MwPopup.show_delay, 500);
      options.hide_delay = this.first_value(options.hide_delay, MwPopup.hide_delay, 200);
      options.cursor_margin = this.first_value(options.cursor_margin, MwPopup.cursor_margin, 5);
    ...
    */
    ...
    bring_to_front: function(event) {
      ...
      if (Number(this.popup.style.zIndex) < MwPopup.zIndex - 1) {
        this.popup.style.zIndex = MwPopup.zIndex++;
      }
    },
    ...
    show: function(event) {
      ...
      Element.setStyle(this.popup, { top: pos.y, left: pos.x, zIndex: MwPopup.zIndex++ });
      this.is_open = true;
    ...
    */
    ...
    show_overlay: function() {
      if (!MwPopup.overlay) {
        ...
        MwPopup.overlay = overlay;
        MwPopup.overlay_levels = [];
      }
      MwPopup.overlay.style.height = this.get_page_dimensions().height + 'px';
      var z = MwPopup.zIndex++;
      MwPopup.overlay.style.zIndex = z;
      MwPopup.overlay_levels.push(z);
      if ( MwPopup.overlay_levels.length == 1) { // Opening the first modal popup.
        // Queue the global overlay effect to ensure correct execution order.
        new Effect.Appear(MwPopup.overlay,
    ...
    hide_overlay: function() {
      MwPopup.overlay_levels.pop();
      var z = MwPopup.overlay_levels.pop();
      if (z) { // There is another modal popup at a lower level so move the overlay back.
        ...
        // Queue the global overlay effect to ensure correct execution order.
        new Effect.Fade(MwPopup.overlay,
