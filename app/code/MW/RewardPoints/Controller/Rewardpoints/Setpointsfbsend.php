...
class Setpointsfbsend extends \MW\RewardPoints\Controller\Rewardpoints
{
	public function execute()
	{
		...
		if ($customer) {
			...
			$sendingUrl = $dataHelper->getLinkShareFacebook($pageUrl);
			...
			if ($dataHelper->checkCustomerMaxBalance($customerId, $store->getCode(), $results[0])) {
				...
				if ($points > 0) {
					...
					$transactions = $this->_objectManager->get(
						...
						->addFieldToFilter('type_of_transaction', Type::SEND_FACEBOOK)
						->addFieldToFilter('transaction_detail', $sendingUrl)
					...
					if (sizeof($transactions) == 0) {
						...
						$historyData = [
							'type_of_transaction' => Type::SEND_FACEBOOK,
							'amount' => (int) $points,
							'balance' => $_customer->getMwRewardPoint(),
							'transaction_detail' => $sendingUrl,
