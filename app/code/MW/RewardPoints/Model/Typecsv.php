    ...
    public function getTransactionDetail($type, $detail = null, $status = null, $is_admin = false)
    {
    	...
    	switch($type) {
            ...
            case self::CUSTOM_RULE:
                $detail_new = $this->_activerulesFactory->create()->load($detail)->getRuleName();
                $result     = __("%1", $detail_new);
                break;
    		...
    		case self::SUBMIT_PRODUCT_REVIEW:
				...
				$result = __("Reward for Posting Product Review %1", $object->getName());
    			break;
    		case self::PURCHASE_PRODUCT:
    			$_detail = explode('|',$detail);
    			$product_id = $_detail[0];
    			$object = $this->_productFactory->create()->load($product_id);
    			$result = __("Reward for purchasing product %1 in order #%2",$object->getName(),$_detail[1]);
    			break;
    		case self::INVITE_FRIEND:
    			$result = __("Reward Referral Visitors: %1",$detail);
    			break;
    		case self::FRIEND_REGISTERING:
    			$object = $this->_customerFactory->create()->load($detail);
    			$result = __("Reward Referral Sign-Ups: %1",$object->getEmail());
    			break;
    		case self::FRIEND_FIRST_PURCHASE:
    			$detail = explode('|',$detail);
    			$object = $this->_customerFactory->create()->load($detail[0]);
    			$result = __("Reward for the first purchase of friend %1",$object->getEmail());
    			break;
    		case self::FRIEND_NEXT_PURCHASE:
    			$detail = explode('|',$detail);
    			$object = $this->_customerFactory->create()->load($detail[0]);
    			$result = __("Reward for purchase of a friend %1",$object->getEmail());
    			break;
    		case self::RECIVE_FROM_FRIEND:
    			$object = $this->_customerFactory->create()->load($detail);
    			$result = __("Receive points from friend %1",$object->getEmail());
    			break;
    		case self::SEND_TO_FRIEND:
    			...
    			$result = __("Send points to friend %1",$email);
    			break;
    		case self::CHECKOUT_ORDER:
    			$result = __("Reward for checkout order #%1",$detail);
    			break;
    		case self::CHECKOUT_ORDER_NEW:
    			$_detail = explode('||',$detail);
    			$result = __("Reward for checkout order #%1",$_detail[0]) . " ";
    			$_detail_rules = [];
    			$_detail_products = [];
    			...
    			foreach ($_detail_rules as $_detail_rule) {
                    ...
                    $result .= __(",+%1 points (%2)", $_detail_rule_child[0], $_detail_rule_child[1]);
                }
    			foreach ($_detail_products as $_detail_product) {
    				$_detail_product_child = explode('|',$_detail_product);
    				$result .= __(",+%1 points for product: %2",$_detail_product_child[0],$_detail_product_child[1]);
    			}
    			break;
    		...
    		case self::USE_TO_CHECKOUT:
    			$result = __("Use to checkout order #%1",$detail);
    			break;
    		...
    		case self::SEND_TO_FRIEND_EXPIRED:
    			$result = __("The sending points to friend %1 was expired",$detail);
    			break;
    		case self::ORDER_CANCELLED_ADD_POINTS:
    			$result = __("Restore spent points for cancelled order #%1",$detail);
    			break;
    		case self::REFUND_ORDER_ADD_POINTS:
    			$result = __("Restore spent points for refunded order #%1",$detail);
    			break;
    		case self::REFUND_ORDER_SUBTRACT_POINTS:
    			$result = __("Subtract reward points for refunded order #%1",$detail);
    			break;
    		case self::REFUND_ORDER_SUBTRACT_PRODUCT_POINTS:
    			$_detail = explode('|',$detail);
    			$product_id = $_detail[0];
    			$object = $this->_productFactory->create()->load($product_id);
    			$result = __("Subtract earned points for product %1 (refunded)", $object->getName());
    			break;
    		case self::REFUND_ORDER_FREND_PURCHASE:
    			$detail = explode('|',$detail);
    			$object = $this->_customerFactory->create()->load($detail[0]);
    			$result = __("Subtract earned points for purchase of friend %1 (refunded)",$object->getEmail());
    			break;
    		case self::LIKE_FACEBOOK:
    			$result = __("Reward for Facebook Like: (<b>%1</b>)",$detail);
    			break;
    		case self::SEND_FACEBOOK:
    			$result = __("Reward for Facebook Share: (<b>%1</b>)",$detail);
    			break;
