define([
    ...
    function ($, actions) {
        ...
        $.widget('mage.iwdOrderManagerInvoiceInfo', $.mage.iwdOrderManagerActions, {
            options: {
                urlUpdate: '',
                urlForm: '',
            ...
            getLoadFromData:function(){
                var id = this.getCurrentInvoiceId();
                return {'form_key':FORM_KEY, 'invoice_id':id};
            },

            getConfirmUpdateData:function(){
                var id = this.getCurrentInvoiceId();
                var data = {'form_key':FORM_KEY, 'invoice_id':id};
                $('#order_information_form').serializeArray().map(function(x){data[x.name] = x.value;});
                return data;
            },

            getCurrentInvoiceId:function(){
                var VRegExp = new RegExp(/invoice_id\/([0-9]+)/);
                var VResult = window.location.href.match(VRegExp);
                return VResult[1];
            }
        });

        return $.mage.iwdOrderManagerInvoiceInfo;
    });